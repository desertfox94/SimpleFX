...
class Properties {
	...
	companion object {
		...
		fun itemsOf(control: Any): ObjectProperty<Any> {
			if (control is ListView<out Any>) {
				return control.itemsProperty() as ObjectProperty<Any>
			} else if (control is TableView<out Any>) {
				return control.itemsProperty() as ObjectProperty<Any>
			} else {
				throw ControlBindingNotSupported(control::class.java.getSimpleName())
			}
